    ...
    <section>
        <div class="row mx-auto py-4 d-flex justify-content-center">
            <span class="mb-4 col-20">
                <h1 class="py-2 text-center text-dark mt-4 fs-2 rounded fw-bold text-uppercase">Historial de solicitudes de participantes de talleres del Corredor Biologico
                  </h1>
            </span>
                            ...
                            <table id="dataTable" class="table table-hover  align-items-center">
                                ...
                                <tbody>
                                    @foreach ($historialParticipantes as $participante)
                                        ...
                                        <input value="{{$participante->id}}" name="id" class="id" id="id" type="hidden">
                                            ...
                                            @if ( $participante->estado === 'Aprobado')
                                                <td name="tipo_persona" class="budget">
                                                    <span class="bg-success text-light rounded-pill p-2"> {{ $participante->estado }} </span>
                                                </td>
                                            @else
                                                <td name="tipo_persona" class="budget">
                                                    <span class="bg-danger text-light rounded-pill p-2"> {{ $participante->estado }} </span>
                                                </td>
                                            @endif
