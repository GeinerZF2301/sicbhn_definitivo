                    ...
                    <div class="table-responsive">
                      <table id="dataTable" class="table table-hover align-items-center">
                        ...
                        <tbody>
                          @foreach ($tipo_personas as $tipo_persona)
                          <tr class="">
                            <td class="w-auto id" name="id" class="budget">{{ $tipo_persona->id }}</td>
                            <td class="w-auto" name="tipo_persona" class="budget">{{ $tipo_persona->tipo_persona }}</td>
                            <td class="w-auto" name="descripcion" class="budget">{{ $tipo_persona->descripcion }}</td>
                            <td class="w-auto">
                              <div class="d-flex flex-column justify-content-between align-items-center">
                                @can('editar-TipoPersona')
                                <button type="button" data-bs-toggle="modal" data-bs-target="#editModal" class="editBtn btn btn-warning btn-sm rounded mb-2">Editar</button>
                                @endcan
                                <button class="showBtn btn btn-info btn-sm rounded mb-2" data-bs-toggle="modal" data-bs-target="#showModal">Ver más</button>
                                @can('borrar-TipoPersona')
                                <button type="submit" class="deleteBtn btn btn-danger btn-sm rounded">Eliminar</button>
                                @endcan
                              </div>
                            </td>
                          </tr>
                      ...
                      <div class="table-responsive">
                        <table id="dataTable" class="table table-hover align-items-center">
                          ...
                          <tbody>
                            @foreach ($tipo_personas as $tipo_persona)
                            <tr>
                              <td class="id" name="id" class="budget">{{ $tipo_persona->id }}</td>
                              ...
                              <td>
                                <div class="d-flex justify-content-center">
                                  ...
                                  @can('borrar-TipoPersona')
                                  <button type="submit" class="deleteBtn shadow rounded btn btn-danger">Eliminar</button>
                                  @endcan
                                </div>
                              </td>
                            </tr>
