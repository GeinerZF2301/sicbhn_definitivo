$(document).ready(function () {
    ...
    $("#create-button").click(function () {
        let method = "create";
        $("#create-modal-title").html("Crear un taller");
    ...
    $('#tipo_taller').on("change", function(){
        if($(this).val() == "otro"){
            $('#otroInput').show();           
        }else{
            $('#otroInput').hide();
    ...
    $('#otroBtn').on('click', function(e){
        ...
        $('#tipo_taller').append(nuevaOpcion);
        $('#otro_tipo').val(' ');
        $('#otro_tipo').hide();
    });
        ...
        $.ajax({
            ...
            error: function (error) {
                if (error.status === 422) {
                    ...
                    if (errors.hasOwnProperty("tipo_taller")) {
                        $("#tipo_taller_error").html(errors.tipo_taller[0]);
                    }
                    if (errors.hasOwnProperty("cupo")) {
                        $("#cupo_error").html(errors.cupo[0]);
                    }
    ...
    $(".editBtn").click(function (e) {
        let url = '/talleres/{id}/edit';
        let method = "edit";
        e.preventDefault();
        $("#edit-modal-title").html("Actualizar Taller");
        ...
        $.ajax({
            ...
            error: function (error) {
                if (error.status === 422) {
                    ...
                    if (errors.hasOwnProperty("tipo_taller")) {
                        $("#edit_tipo_taller_error").html(errors.tipo_taller[0]);
                    }
                    if (errors.hasOwnProperty("cupo")) {
                        $("#edit_cupo_error").html(errors.cupo[0]);
                    }
                    if (errors.hasOwnProperty("estado")) {
...
$(".showBtn").click(function (e) {
    e.preventDefault();
    let url = "/talleres/{id}/show";
    let method = "show";
    let id = $(this).closest("tr").find("td.id").text();
    $("#show-modal-title").html("Información de la Taller");
    ...
    $.ajax({
        ...
        success: function (response) {
            if (method === "edit") {
                ...
                $("#edit_tipo_taller").val(response.tipo_taller);
                $("#edit_cupo").val(response.cupo);
                ...
                $("#show_tipo_taller").val(response.tipo_taller);
                $("#show_cupo").val(response.cupo);
