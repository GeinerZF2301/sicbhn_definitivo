$(document).ready(function() {
    ...
    obtenerArticulos(method);
   
    //Peticion o solicitud POST para calcular la edad de la Persona
   
    $('#create-button').click(function() {
        let method = 'create';
        $('#create-modal-title').html('Crear un Registro de Donacion en especie');
        $('#saveBtn').html('Guardar');
        $('#fecha_donacion').val('').on("change", function () {
            limpiarMensagesValidacion();
        });
        $('#descripcion').val('').on("change", function () {
            limpiarMensagesValidacion();
        });
        $('#fecha_recibido').val('').on("change", function () {
            limpiarMensagesValidacion();
        });
        $('#monto').val('').on("change", function () {
            limpiarMensagesValidacion();
        });
        $('#persona_donante_id').val('').on("change", function () {
            limpiarMensagesValidacion();
        });
    });
            ...
            error: function(error) {
                if (error.status === 422) {
                    ...
                    if (errors.hasOwnProperty("articulo_id")) {
                        $("#articulo_id_error").html(errors.articulo_id[0]);
                    }
    ...
    obtenerArticulos(editMethod);
    
// Logica del cliente que realiza un GET para el metodo Edit Tipo Persona
    $(".editBtn").click(function(e) {
        let url = "/donacionEspecie/{id}/edit";
        let method = 'edit';
        e.preventDefault();
        $('#edit-modal-title').html('Actualizar Registro');
...
//Logica del cliente que realiza una peticion POST al servidor para realizar un Update del registro
    ...
    $('.updateBtn').click(function(e){
        ...
        $.ajax({
            type: "POST",
            url: "/donacionEspecie/update/" + idRequest,
            data: formEdit,
            processData: false,
            contentType: false,
            success: function (response) {
    ...
    $(".showBtn").click(function(e) {
        e.preventDefault();
        let url = "/donacionEspecie/{id}/show";
        let method = 'show';
    ...
    $.ajax({
        ...
        success: function (response) {
            if(method === 'edit'){
                ...
                $('#edit_articulo_id').val(response.articulo_id);
            }else{
                ...
                $('#show_articulo_id').val(response.articulo_id);
            }
        },
        error: function(error){
            console.log(error)
...
function obtenerArticulos(method){
    let select = '';
    let option = '';
    $.ajax({
        type: 'GET',
        ...
        success: function (response) {
            if(method === 'create'){
                select  = $('#articulo_id')
                $.each(response, function(index,item){
                option = $('<option></option').attr('value', item.id).text(item.nombre);
                    $(select).append(option);
                }); 
            }
            else if(method === 'edit'){
                select  = $('#edit_articulo_id')
                $.each(response, function(index,item){
                    option = $('<option></option').attr('value', item.id).text(item.nombre);
                    $(select).append(option);
                }); 
