...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(80, 10, utf8_decode("REPORTE DE GESTIÓN DE VOLUNTARIADOS"), 0, 1, 'C', 0);
        $this->Ln(7);
	...
	function CheckPageBreak($h, $setX) {
		...
		if ($this->GetY() + $h > $this->PageBreakTrigger) {
            ...
            $this->Cell(60, 10, utf8_decode('UBICACIÓN'), 1, 0, 'C', 1);
            $this->Cell(40, 10, utf8_decode('FECHA'), 1, 0, 'C', 1);
            $this->Cell(40, 10, utf8_decode('HORA'), 1, 1, 'C', 1);
...
// -----------ENCABEZADO------------------
            ...
            $pdf->Cell(40, 10, utf8_decode('HORA'), 1, 1, 'C', 1);
			$pdf->SetFont('Arial', '', 12);
...
for ($i = 0; $i < count($data); $i++) {
    ...
    $pdf->Row(array($i + 1, $data[$i]['nombre'], utf8_decode($data[$i]["ubicacion"]), utf8_decode($data[$i]["descripcion"]), 
    utf8_decode($data[$i]["fecha"]), utf8_decode($data[$i]["hora"])), 10);
}
    // cell(ancho, largo, contenido,borde?, salto de linea?)
    
    $pdf->Output('Reporte de Voluntaridos.pdf', 'I');//nombreDescarga, Visor(I->visualizar - D->descargar)
    ?>