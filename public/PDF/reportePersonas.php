...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(80, 10, utf8_decode("REPORTE DE GESTIÓN DE PERSONAS"), 0, 1, 'C', 0);
        $this->Ln(7);
	...
	function CheckPageBreak($h, $setX) {
		...
		if ($this->GetY() + $h > $this->PageBreakTrigger) {
            ...
            $this->Cell(60, 10, utf8_decode('APELLIDOS'), 1, 0, 'C', 1);
            $this->Cell(60, 10, utf8_decode('TIPO DE IDENTIFICACIÓN'), 1, 0, 'C', 1);
            $this->Cell(40, 10, utf8_decode('IDENTIFICACIÓN'), 1, 0, 'C', 1);
            $this->Cell(40, 10, utf8_decode('EDAD'), 1, 1, 'C', 1);
...
// -----------ENCABEZADO------------------
            ...
            $pdf->Cell(60, 10, utf8_decode('TIPO DE IDENTIFICACIÓN'), 1, 0, 'C', 1);
            $pdf->Cell(40, 10, utf8_decode('IDENTIFICACIÓN'), 1, 0, 'C', 1);
            $pdf->Cell(40, 10, utf8_decode('EDAD'), 1, 1, 'C', 1);
			$pdf->SetFont('Arial', '', 12);
...
$pdf->SetWidths(array(20, 50, 60, 60, 40, 40)); //???
// esto no lo mencione en el video pero también pueden poner la alineación de cada COLUMNA!!!


for ($i = 0; $i < count($data); $i++) {

    $pdf->Row(array($i + 1, $data[$i]['nombre'], utf8_decode($data[$i]["apellidos"]), utf8_decode($data[$i]["tipo_identificacion"]), 
    utf8_decode($data[$i]["numero_identificacion"]), utf8_decode($data[$i]["edad"])), 10);
}
